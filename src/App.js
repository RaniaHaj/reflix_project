...
function App() {
  ...
  let [budget, setBudget] = useState(10.0);

  const changeMovieStatus = (movie) => {
    let newMovies = [...movies];
    const matchMovie = newMovies.filter((m) => m.id === movie.id)[0];
    matchMovie.isRented = matchMovie.isRented ? false : true;
    setMovies(newMovies);
  };
